...
from hashfunc import hash,md5_hash
...
class IS_IN_SET(Validator):
    ...
    def __init__(
        ...
        if isinstance(theset, dict):
            self.theset = [xmlescape(item) for item in theset]
            self.labels = theset.values()
        elif theset and isinstance(theset, (tuple,list)) \
            and isinstance(theset[0], (tuple,list)) and len(theset[0])==2:
            self.theset = [xmlescape(item) for item,label in theset]
            self.labels = [xmlescape(label) for item,label in theset]
        else:
            self.theset = [xmlescape(item) for item in theset]
            self.labels = labels
        self.error_message = error_message
